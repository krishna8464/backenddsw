...
//Feedback post form user side is done ------->localhost:6060/user/payment
    ...
    try {
        ...
        if(decoded){
           ...
           const allcartdata =  await CartModel.find({userID:userid})
            ...
            res.status(200).send({"msg":"Payment Successfull"})
        }else{
            res.status(200).send("No items in the cart")
        }
        }else{
            res.status(401).send({"msg":"Authorization Requiread"})
        }
    } catch (error) {
        res.sendStatus(404)
    }
