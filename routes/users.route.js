            ...
            if(req.body.password){
                bcrypt.hash(req.body.password, 5,async(err,val)=>{
                    if(err){
                        ...
                        res.status(200).send({"msg":"Updated Successfully","id":userid})
                    }
                })
            }else{
            await UserModel.findByIdAndUpdate({_id:userid},body);
...
//admin can get user with his id ----------->localhost:6060/user/admin/:userid
userRoute.get("/admin/:userid",async(req,res)=>{
    try {
       let user =  await UserModel.find({_id:req.params.userid});
       let {_id,username,email,avatar,password} = user[0]
        res.status(200).send({username,email,avatar})
    } catch (error) {
        res.status(400).send({"msg":"something wetn wrong"})
    }
})
